...
public class JCaptchaServiceImpl extends DefaultManageableImageCaptchaService implements CaptchaService {
	...
	private DefaultManageableImageCaptchaService imageService;

	public JCaptchaServiceImpl() {
		imageService = new DefaultManageableImageCaptchaService(MIN_STORAGE_DELAY, MAX_STORE_SIZE, STORE_LOAD_BEFORE_GARBAGE_COLLECTION);
		...
		ComposedWordToImage word2image = new ComposedWordToImage(
				new RandomFontGenerator(FONT_SIZE, FONT_SIZE, new Font[]{getFont()}),
				new TransparentBackgroundGenerator(),
				new SimpleTextPaster(MIN_WORD_LENGTH, MAX_WORD_LENGTH, ORANGE));
		GimpyFactory factory = new GimpyFactory(wordgen, word2image, false);
		GenericCaptchaEngine engine = new GenericCaptchaEngine(new CaptchaFactory[]{factory});
		imageService.setCaptchaEngine(engine);
	}

	private Font getFont() {
		try {
			return Font.createFont(Font.PLAIN, getClass().getResourceAsStream(ARIAL_FONT_PATH));
		} catch (IOException x) {
	...
	public void writeImage(OutputStream out, String captchaId, Locale locale) throws IOException {
		try {
			BufferedImage image = imageService.getImageChallengeForID(captchaId, locale);
	...
	public boolean validateImageResponse(String captchaId, String response) {
		try {
			return imageService.validateResponseForID(captchaId, response);
	...
	private class TransparentBackgroundGenerator implements BackgroundGenerator {
